...
{
    /// <summary>
    /// Validates and quotes a MySQL identifier to prevent SQL injection.
    /// </summary>
    private static string QuoteIdentifier(string identifier)
    {
        ...
        if (!Regex.IsMatch(identifier, @"^[a-zA-Z_$][a-zA-Z0-9_$ ]*$"))
            throw new ArgumentException($"Invalid MySQL identifier: {identifier}", nameof(identifier));
        return $"`{identifier}`";
    }
    ...
    /// <summary>
    /// Fetches a batch of rows from a MySQL table using LIMIT/OFFSET.
